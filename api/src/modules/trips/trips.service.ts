...
export class TripService {
  ...
  async createTrip(trip: Partial<Trip>): Promise<Trip> {
    ...
    locationFrom = await this.locationRepository.findOne({
      where: { id: trip?.locationFromId },
    });

    if (!locationFrom) {
      throw new ValidationError('Starting location not found', {
        referenceId: trip?.locationFromId,
    ...
    createdBy = await this.userRepository.findOne({
      where: { id: trip?.createdById },
    });

    if (!createdBy) {
      throw new NotFoundError('Created by not found', {
        referenceId: trip?.createdById,
    ...
    // CREATE TRIP
    const newTrip = await this.tripRepository.save({
      ...trip,
      referenceId,
      locationFrom: locationFrom ?? undefined,
      locationTo: locationTo ?? undefined,
      createdBy,
    });
   ...
   * START TRIP
   */
  async startTrip(id: UUID, _metadata?: { createdById?: UUID }): Promise<Trip> {
    const trip = await this.tripRepository.findOne({
      where: { id: id as UUID },
      relations: {
        locationFrom: true,
  ...
  async fetchNearbyTrips({
    ...
    const mapTripToResult = (
      ...
      availableCapacity:
        ...
        (inProgressCountByTrip.get(trip.id as UUID) ?? 0),
      totalCapacity: trip.totalCapacity ?? 0,
    ...
    try {
      ...
      const sortedByDistance = candidateTrips
        .map((trip) => {
          const coordinates = (
            trip.locationFrom?.address as { coordinates?: number[] }
          )?.coordinates;
  ...
  async quickJoinTrip({
    ...
    const userTripPayload: Partial<UserTrip> = {
      ...
      entranceLocation:
        entranceLocation as unknown as UserTrip['entranceLocation'],
      startTime: new Date(),
      status: UserTripStatus.IN_PROGRESS,
